...
public class TLSManager {
    ...
    public var secTrustClosure: ((SecTrust)->Bool)?
    ...
    func getNWParams(_ queue: DispatchQueue, timeout: Int = 60) -> Result<NWParameters> {
        ...
        sec_protocol_options_set_verify_block(options.securityProtocolOptions, { [weak self] (_, sec_trust, completionHandler) in
            ...
            let accepted = Self.trustDecision {
                self?.secTrustClosure?(serverTrust) ?? false
            }
    ...
    static func trustDecision(_ evaluator: (() -> Bool)?) -> Bool {
